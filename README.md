...
**Day 1**
1. CREATE (@PostMapping)
   1. @RequestBody로 필수적으로 유저 **아이디와 비밀번호** 부수적으로 전화번호, 이메일, 주소 정보를 JSON 형식으로 받아옴.
   2. 유저 등록 전 겹치는 유저 아이디가 있는지 검증
   3. 비밀번호는 암호화 후 유저 등록
2. CREATE JWT (@PostMapping)
   1. @RequestBody로 유저 **아이디와 비밀번호**를 입력 받음.
   2. 받은 정보를 통해 JWT 발급